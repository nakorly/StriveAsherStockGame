-- Fixed Database Setup Script for Stock Trading Game
-- This script resolves the infinite recursion issue in admin policies
...
-- =============================================================================
-- 3. CREATE RLS POLICIES (Fixed to avoid infinite recursion)
-- =============================================================================
...
-- Allow initial admin creation (anyone can insert if no super admin exists)
DROP POLICY IF EXISTS "Allow initial admin creation" ON admin_roles;
CREATE POLICY "Allow initial admin creation" ON admin_roles
  FOR INSERT WITH CHECK (
    NOT EXISTS (SELECT 1 FROM admin_roles WHERE role = 'SUPER_ADMIN') OR
    EXISTS (SELECT 1 FROM admin_roles ar WHERE ar.user_id = auth.uid() AND ar.role = 'SUPER_ADMIN')
  );
...
-- Function to check if current user is admin (safe version)
...
BEGIN
  ...
  INSERT INTO leaderboard (user_id, username, display_name, total_value, total_gain_loss, total_gain_loss_percent, rank)
  SELECT 
    p.id as user_id,
    ...
    ROW_NUMBER() OVER (ORDER BY (COALESCE(p.balance, 0) + COALESCE(portfolio_value.total_value, 0)) DESC) as rank
  FROM profiles p
  LEFT JOIN (
    SELECT 
      user_id,
